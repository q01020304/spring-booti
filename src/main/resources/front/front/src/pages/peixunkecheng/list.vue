...
<div>
	...
	<div class="list-preview" :style='{"width":"86%","margin":"10px auto","position":"relative"}'>
		...
		<div class="category-3" :style='{"width":"100%","padding":"10px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
			<div class="item" :class="swiperIndex == '-1' ? 'active' : ''" @click="getList(1, '全部')" :plain="isPlain">
				<div :style='{"color":"inherit","fontSize":"14px","display":"inline-block"}'>全部</div>
			</div>
			<div class="item" :class="swiperIndex == index ? 'active' : ''" v-for="(item, index) in fenlei" :key="index" @click="getList(1, item[feileiColumn], 'btn' + index)" :ref="'btn' + index" plain>
				<img v-if="item.image" :style='{"width":"34px","margin":"0 5px 0 0","objectFit":"cover","display":"inline-block","height":"34px"}' :src="baseUrl + (item.image?item.image.split(',')[0]:'')">
				<div :style='{"color":"inherit","fontSize":"14px","display":"inline-block"}'>{{item[feileiColumn]}}</div>
			</div>
		</div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
	  ...
	  <el-button v-if="btnAuth('peixunkecheng','新增')" :style='{"cursor":"pointer","border":"1px solid #769589","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#769589","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"35px","height":"35px"}' type="primary" @click="add('/index/peixunkechengAdd')"><i v-if="true" :style='{"color":"#769589","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="sort_view" :style='{"margin":"10px","background":"#769589","justifyContent":"space-around","display":"flex"}'>
		<el-button :style='{"border":"0","padding":"0 15px","margin":"0 5px","borderRadius":"8px","background":"#769589"}' @click="sortClick('clicknum')">
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#FFFFFF"}' class="icon iconfont icon-xiaoliang13" v-if="sortType!='clicknum'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#FFFFFF"}' class="icon iconfont icon-xiaoliang13" v-else-if="sortType=='clicknum'&&sortOrder=='desc'"></span>
			<span :style='{"margin":"0 2px 0 0","lineHeight":"40px","fontSize":"14px","color":"#FFFFFF"}' class="icon iconfont icon-xiaoliang13" v-else-if="sortType=='clicknum'&&sortOrder=='asc'"></span>
			<span :style='{"color":"#FFFFFF","lineHeight":"40px","fontSize":"14px"}'>点击量</span>
		</el-button>
	</div>
	<div class="list" :style='{"margin":"0 0 10px","background":"#fff"}'>
		...
		<div class="list2 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"0","boxShadow":"0px 4px 10px 0px #769589","margin":"10px 1% 16px","flexWrap":"wrap","background":"#fff","display":"flex","width":"31%","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				...
				<div class="item-info" :style='{"width":"100%","padding":"10px 10px 16px","overflow":"hidden","display":"inline-block","height":"auto"}'>
					<div :style='{"padding":"0 5px","lineHeight":"20px","fontSize":"14px","color":"#769589","display":"inline-block"}' class="name ">{{item.kechengmingcheng}}</div>
					<div :style='{"padding":"0 5px","lineHeight":"20px","fontSize":"14px","color":"#769589","display":"inline-block"}' class="name ">{{item.jiaoshixingming}}</div>
					<div :style='{"padding":"0 5px","lineHeight":"20px","fontSize":"14px","color":"#769589","display":"inline-block"}' class="name ">课程价格:{{item.kechengjiage}}</div>
					<div v-if="item.price" :style='{"padding":"0 5px","lineHeight":"24px","fontSize":"14px","color":"red"}' class="price"><span :style='{"fontSize":"12px"}'>￥</span>{{item.price}}</div>
					...
					<div :style='{"padding":"0 5px","display":"inline-block"}'>
					  ...
					  <span :style='{"color":"#666","lineHeight":"1.5","fontSize":"12px"}'>{{item.jiaoshigonghao}}</span>
					</div>
					<div :style='{"padding":"0 5px","display":"inline-block"}'>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`peixunkecheng/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/peixunkechengDetail', query: params});
      },
