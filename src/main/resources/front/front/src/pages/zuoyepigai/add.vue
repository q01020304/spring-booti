...
<div :style='{"width":"1200px","padding":"20px","margin":"10px auto 20px","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="完成图片" v-if="type!='cross' || (type=='cross' && !ro.wanchengtupian)" prop="wanchengtupian">
            <file-upload
            tip="点击上传完成图片"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' class="upload" v-else label="完成图片" prop="wanchengtupian">
                ...
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.wanchengtupian.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="作业名称" prop="zuoyemingcheng">
            <el-input v-model="ruleForm.zuoyemingcheng" 
                placeholder="作业名称" clearable :disabled=" false  ||ro.zuoyemingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="作业成绩" prop="zuoyechengji">
			<el-input-number v-model="ruleForm.zuoyechengji" placeholder="作业成绩" :readonly="ro.zuoyechengji"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="批改时间" prop="pigaishijian">
              <el-date-picker
				  :disabled=" false  ||ro.pigaishijian"
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"#fff"}' label="评语" prop="pingyu">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="评语"
...
<script>
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          ...
          wanchengtupian: [
            { required: true, message: '完成图片不能为空', trigger: 'blur' },
          ],
          zuoyemingcheng: [
    ...
    created() {
      ...
      this.ruleForm.pigaishijian = this.getCurDateTime()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='wanchengtupian'){
              this.ruleForm.wanchengtupian = obj[o].split(",")[0];
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`zuoyepigai/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('zuoyepigai/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`zuoyepigai/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      wanchengtupianUploadChange(fileUrls) {
          this.ruleForm.wanchengtupian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
