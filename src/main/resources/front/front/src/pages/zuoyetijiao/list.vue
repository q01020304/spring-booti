...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
	  ...
	  <el-button v-if="btnAuth('zuoyetijiao','新增')" :style='{"cursor":"pointer","border":"1px solid #769589","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#769589","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"35px","height":"35px"}' type="primary" @click="add('/index/zuoyetijiaoAdd')"><i v-if="true" :style='{"color":"#769589","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list2 index-pv1" :style='{"width":"100%","padding":"0 10px","flexWrap":"wrap","background":"#fff","display":"flex","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"0","boxShadow":"0px 4px 10px 0px #769589","margin":"10px 1% 16px","flexWrap":"wrap","background":"#fff","display":"flex","width":"31%","fontSize":"0","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<div :style='{"width":"100%","padding":"0","overflow":"hidden","height":"250px"}'>
					<img @click.stop="imgPreView(item.wanchengtupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.wanchengtupian && item.wanchengtupian.substr(0,4)=='http'" :src="item.wanchengtupian.split(',')[0]" class="image" />
					<img @click.stop="imgPreView(baseUrl + (item.wanchengtupian?item.wanchengtupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.wanchengtupian?item.wanchengtupian.split(',')[0]:'')" class="image" />
				</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.selectOptionsList.push({name:'批改状态',list:this.pigaizhuangtaiOptions,tableName: 'pigaizhuangtai',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.pigaizhuangtai != '') searchWhere.pigaizhuangtai = this.formSearch.pigaizhuangtai;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`zuoyetijiao/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/zuoyetijiaoDetail', query: params});
      },
