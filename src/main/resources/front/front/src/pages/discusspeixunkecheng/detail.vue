...
<div>
	<div :style='{"border":"1px solid #769589","padding":"10px","margin":"10px auto","borderRadius":"0","display":"flex","width":"86%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/discusspeixunkecheng"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"padding":"20px 0","position":"relative","order":"1"}'>
			...
			<div class="info" :style='{"padding":"0 0 40px","margin":"0","background":"#F9F8F8","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"0 10% 0 10%","margin":"0 0 10px 0","background":"#F9F8F8","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"right","width":"180px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>关联表id</div>
					<div  :style='{"padding":"8px 10px 0","boxShadow":"0 1px 0 #999","color":"#000","flex":"1","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.refid}}</div>
				</div>
				<div class="item" :style='{"padding":"0 10% 0 10%","margin":"0 0 10px 0","background":"#F9F8F8","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"right","width":"180px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>用户名</div>
					<div  :style='{"padding":"8px 10px 0","boxShadow":"0 1px 0 #999","color":"#000","flex":"1","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.nickname}}</div>
				</div>
				<div class="item" :style='{"padding":"0 10% 0 10%","margin":"0 0 10px 0","background":"#F9F8F8","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"right","width":"180px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>评论内容</div>
					<div  :style='{"padding":"8px 10px 0","boxShadow":"0 1px 0 #999","color":"#000","flex":"1","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.content}}</div>
				</div>
				<div class="item" :style='{"padding":"0 10% 0 10%","margin":"0 0 10px 0","background":"#F9F8F8","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#000","textAlign":"right","width":"180px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>回复内容</div>
					<div  :style='{"padding":"8px 10px 0","boxShadow":"0 1px 0 #999","color":"#000","flex":"1","fontSize":"14px","lineHeight":"24px","height":"auto"}'>{{detail.reply}}</div>
				</div>
				<div class="btn" :style='{"padding":"0 0 0 180px","margin":"20px 0 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#769589","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discusspeixunkecheng','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#769589","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discusspeixunkecheng','删除')" @click="delClick">删除</el-button>
					<!-- hasChat $hasChat -->
				</div>
			</div>
		</div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        tablename: 'discusspeixunkecheng',
        baseUrl: '',
        breadcrumbItem: [
          {
            name: '培训课程评论表'
    ...
    methods: {
      ...
      getDiscussList(page) {
        this.$http.get('discussdiscusspeixunkecheng/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussdiscusspeixunkecheng/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussdiscusspeixunkecheng/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		  ...
		  this.$http.post('discusspeixunkecheng/update',this.detail).then(res=>{
		...
		// 修改
		editClick(){
			this.$router.push(`/index/discusspeixunkechengAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		delClick(){
			this.$confirm('是否删除此培训课程评论表？')
			  .then(_ => {
			    this.$http.post('discusspeixunkecheng/delete', [this.detail.id]).then(res => {
			      if (res.data.code == 0) {
			        this.$message({
			          type: 'success',
			          message: '删除成功!',
