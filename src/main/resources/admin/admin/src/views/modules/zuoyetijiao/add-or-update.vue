	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.wanchengtupian" label="完成图片" prop="wanchengtupian">
					<file-upload
						tip="点击上传完成图片"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.wanchengtupian?ruleForm.wanchengtupian:''"
						@change="wanchengtupianUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.wanchengtupian" label="完成图片" prop="wanchengtupian">
					<img v-if="ruleForm.wanchengtupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.wanchengtupian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.wanchengtupian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.tijiaozuoye" label="提交作业" prop="tijiaozuoye">
					<file-upload
						tip="点击上传提交作业"
						action="file/upload"
						:limit="1"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.tijiaozuoye" label="提交作业" prop="tijiaozuoye">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","color":"rgba(255, 255, 255, 1)","outline":"none","borderRadius":"4px","background":"#6aab9c","width":"auto","lineHeight":"40px","fontSize":"14px","fontWeight":"400","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.tijiaozuoye)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="!ruleForm.tijiaozuoye" label="提交作业" prop="tijiaozuoye">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="批改状态" prop="pigaizhuangtai">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="批改状态" prop="pigaizhuangtai">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				tijiaozuoye: [
					{ required: true, message: '提交作业不能为空', trigger: 'blur' },
				],
				tijiaoshijian: [
				],
				pigaizhuangtai: [
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='wanchengtupian'){
							this.ruleForm.wanchengtupian = obj[o];
							this.ro.wanchengtupian = true;
							continue;
						}
						if(o=='zuoyemingcheng'){
				...
				this.ruleForm.pigaizhuangtai = '未批改'; 			}
	...
	if(this.ruleForm.wanchengtupian!=null) {
		this.ruleForm.wanchengtupian = this.ruleForm.wanchengtupian.replace(new RegExp(this.$base.url,"g"),"");
	}


	if(this.ruleForm.tijiaozuoye!=null) {
		this.ruleForm.tijiaozuoye = this.ruleForm.tijiaozuoye.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
    ...
    wanchengtupianUploadChange(fileUrls) {
	    this.ruleForm.wanchengtupian = fileUrls;
    },
