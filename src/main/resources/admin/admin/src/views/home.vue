...
<div class="content" :style='{"minHeight":"100vh","padding":"30px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20240220/927b2d08dcdb48b3af07cef8d1a59f59.png) no-repeat center top / cover","display":"flex","width":"100%"}'>
	<!-- notice -->
	<!-- title -->
	<div class="text" :style='{"width":"100%","margin":"20px auto","fontSize":"24px","color":"rgb(51, 51, 51)","textAlign":"center","fontWeight":"bold"}'>欢迎使用 {{this.$project.projectName}}</div>
	<!-- statis -->
	<div :style='{"width":"100%","margin":"0 0 20px 0","alignItems":"center","justifyContent":"center","display":"flex"}'>
		<div :style='{"border":"1px solid  #f0f0f0","margin":"0 10px","borderRadius":"4px","background":"#ffffff","display":"flex"}' v-if="isAuth('xueyuan','首页总数')">
			...
			<div :style='{"width":"160px","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{xueyuanCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>学员总数</div>
			</div>
		</div>
		<div :style='{"border":"1px solid rgb(240, 240, 240)","margin":"0 10px","borderRadius":"4px","background":"rgb(255, 255, 255)","display":"flex"}' v-if="isAuth('jiaoshi','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","justifyContent":"center","display":"flex","height":"80px"}'>
		...
		<div :style='{"border":"1px solid rgb(240, 240, 240)","margin":"0 10px","borderRadius":"4px","background":"rgb(255, 255, 255)","display":"flex"}' v-if="isAuth('peixunkecheng','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-tongji5" :style='{"color":"rgb(247, 179, 107)","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{peixunkechengCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>培训课程总数</div>
			</div>
		</div>
		<div :style='{"border":"1px solid rgb(240, 240, 240)","margin":"0 10px","borderRadius":"4px","background":"rgb(255, 255, 255)","display":"flex"}' v-if="isAuth('xuexizhongxin','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-tongji5" :style='{"color":"rgb(247, 179, 107)","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{xuexizhongxinCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>学习中心总数</div>
			</div>
		</div>
		<div :style='{"border":"1px solid rgb(240, 240, 240)","margin":"0 10px","borderRadius":"4px","background":"rgb(255, 255, 255)","display":"flex"}' v-if="isAuth('kehouzuoye','首页总数')">
			<div :style='{"width":"80px","alignItems":"center","justifyContent":"center","display":"flex","height":"80px"}'>
				<span class="icon iconfont icon-tongji5" :style='{"color":"rgb(247, 179, 107)","fontSize":"24px"}'></span>
			</div>
			<div :style='{"width":"160px","flexDirection":"column","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"20px","color":"#333","fontWeight":"bold","height":"24px"}'>{{kehouzuoyeCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"16px","color":"#666","height":"24px"}'>课后作业总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type5" :style='{"alignContent":"flex-start","padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto"}'>
		<div id="xueyuanChart1" class="echarts1" v-if="isAuth('xueyuan','首页统计')"></div>
		<div id="xueyuanChart2" class="echarts2" v-if="isAuth('xueyuan','首页统计')"></div>
		<div id="xuexizhongxinChart1" class="echarts3" v-if="isAuth('xuexizhongxin','首页统计')"></div>
		<div id="xuexizhongxinChart2" class="echarts4" v-if="isAuth('xuexizhongxin','首页统计')"></div>
		<div id="zuoyepigaiChart1" class="echarts5" v-if="isAuth('zuoyepigai','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	data() {
		return {
            xueyuanCount: 0,
            jiaoshiCount: 0,
            peixunkechengCount: 0,
            xuexizhongxinCount: 0,
            kehouzuoyeCount: 0,
	...
	mounted(){
		...
		this.getpeixunkechengCount();
		this.getxuexizhongxinCount();
		this.xuexizhongxinChat1();
		this.xuexizhongxinChat2();
		this.getkehouzuoyeCount();
		this.zuoyepigaiChat1();
	},
	methods:{
		// 词云
        ...
        var xueyuanChart1 = echarts.init(document.getElementById("xueyuanChart1"),'macarons');
        this.$http({
            url: "xueyuan/group/banji",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                xueyuanChart1.setOption(option);
				// 动画效果
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    xueyuanChart1.resize();
                };
            }
        });
      })
    ...
    xueyuanChat2() {
      this.$nextTick(()=>{
        ...
        var xueyuanChart2 = echarts.init(document.getElementById("xueyuanChart2"),'macarons');
        this.$http({
            url: "xueyuan/group/xingbie",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                xueyuanChart2.setOption(option);
				
				// 动画效果
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    xueyuanChart2.resize();
                };
            }
        });
      })
		...
		getpeixunkechengCount() {
			this.$http({
				url: `peixunkecheng/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.peixunkechengCount = data.data
				}
			})
		},
		getxuexizhongxinCount() {
			this.$http({
				url: `xuexizhongxin/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.xuexizhongxinCount = data.data
				}
			})
		},
		xuexizhongxinChat1() {
        ...
        var xuexizhongxinChart1 = echarts.init(document.getElementById("xuexizhongxinChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                xuexizhongxinChart1.setOption(option);
				// 动画效果
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    xuexizhongxinChart1.resize();
                };
            }
        });
      })
    ...
    xuexizhongxinChat2() {
      this.$nextTick(()=>{
        ...
        var xuexizhongxinChart2 = echarts.init(document.getElementById("xuexizhongxinChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                xuexizhongxinChart2.setOption(option);
				
				// 动画效果
				this.myChartInterval(1, option.xAxis.data, option.series[0].data, xuexizhongxinChart2)
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    xuexizhongxinChart2.resize();
                };
            }
        });
      })
		...
		getkehouzuoyeCount() {
			this.$http({
				url: `kehouzuoye/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.kehouzuoyeCount = data.data
				}
			})
		},
		zuoyepigaiChat1() {
        ...
        var zuoyepigaiChart1 = echarts.init(document.getElementById("zuoyepigaiChart1"),'macarons');
        this.$http({
            url: "zuoyepigai/sectionStat/zuoyechengji",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    ...
                    xAxis.push(res[i].zuoyechengji);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].zuoyechengji
                    })
                }
                var option = {};
				let titleObj = this.gauge.title
				titleObj.text = '作业成绩分布'
				let tooltipObj = {trigger: 'item',formatter: '{b} : {c}'}
                ...
                // 使用刚指定的配置项和数据显示图表。
                zuoyepigaiChart1.setOption(option);
				// 动画效果
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zuoyepigaiChart1.resize();
                };
            }
        });
      })
